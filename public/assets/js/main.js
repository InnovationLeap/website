...
(function($) {
  ...
  $window.on('load', function() {
    ...
    function syncNavPanel() {
      ...
      const html = (typeof $.fn.navList === 'function') ? $nav.navList() : ''
      if (!html) return false

      let $panel = $('#navPanel')
      if ($panel.length) {
        // Update existing panel content if empty or outdated
        const $navContainer = $panel.children('nav')
    ...
    if (headerNav && 'MutationObserver' in window) {
      const mo = new MutationObserver(function() { syncNavPanel() })
    ...
    if (contentEl && 'MutationObserver' in window) {
      const mo2 = new MutationObserver(function() { adjustMediaAspect() })
